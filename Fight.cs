...
public class Fight
{
    ...
    //绘制血量
    private static void draw_blood1(Graphics g)
    {
        long time = Comm.Time();
    ...
    private static void draw_blood(Graphics g)
    {
        ...
        // 根据战斗目标的类型和索引选择角色位置数组或敌人位置数组
        Point[] positions = turn.target_type == 1 ? player_pos : enemy_pos;
        int targetIndex = turn.target_index;

        // 位置
        draw_blood_x = positions[targetIndex].X - 5;
        draw_blood_y = positions[targetIndex].Y - 120;
